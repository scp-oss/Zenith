...
def run(profile: str, genome_id: str, after_strategy: int, environment_name: str, provider: str, min_pulls: int) -> int:
    ...
    print()
    print("# !!! ОБЯЗАТЕЛЬНЫЙ шаг, не пропускать -- живой инцидент 2026-08-07 (см. README):")
    print("# circular_locked (z2r_lib) держит max strategy= для хоста в памяти ПРОЦЕССА")
    print("# без TTL, вычисляет один раз при первом коннекте. Новый номер strategy=,")
    print("# который процесс не перечитал -- locked=N > закэшированный max -- тихо")
    print("# откатывается на strategy=1, БЕЗ ошибок в логе. set_strategy_cli.sh set")
    print("# сам по себе НЕ горячее применение, а переключатель среди УЖЕ загруженных")
    print("# в память стратегий. Перезапуск обязателен ПЕРЕД переключением, и его нужно")
    print("# подтвердить, не считать, что 'команда где-то отработала':")
    print()
    ...
    print("# завершённым -- см. domain_pool для этого профиля.")
