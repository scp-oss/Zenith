"""Direct-MySQL backend for db.py (ZENITH_DB_MODE=docker -- local buffer
DB, whether isolated or hub-and-spoke-synced). See db_api.py for the
ZENITH_DB_MODE=api backend (no local DB, same function signatures, HTTP
to the panel instead) and db.py for the selector between the two --
main.py/bootstrap.py/compare_control.py/promote.py only ever `import db`,
