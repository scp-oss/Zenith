#!/usr/bin/env bash
# Ограничивает вход на публичный порт панели (тот, что торчит наружу
# через Caddy, см. README "Публикация панели через Cloudflare") только
...
#   0 4 * * * root /opt/z2r_autobench/Zenith/panel/cloudflare_iptables.sh <PORT> >> /var/log/cf-iptables.log 2>&1
set -euo pipefail
...
# был со stale NFQUEUE-правилами, см. CLAUDE.md).
