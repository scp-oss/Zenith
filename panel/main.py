#!/usr/bin/env python3
"""Zenith web-панель: обзор геномов по нодам/провайдерам, история мутаций
конкретных стратегий, read-only статус боевых профилей. Плюс sync API для
удалённых нод (см. sync_api.py). Панель работает НА той же машине, что
локальный orchestrator, и делит с ним ту же MySQL (см. config.py) --
отдельной БД для панели нет.
...
UI. Она только читает (get/max) и отображает то, что orchestrator/main.py
и promote.py уже посчитали -- финальное применение остаётся ручным шагом
человека, как и раньше.

Запуск: см. run.sh / README.md (systemd-юнит panel/zenith-panel.service).
"""
import subprocess
import sys
